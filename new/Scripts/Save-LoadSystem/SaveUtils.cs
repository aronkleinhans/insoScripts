...
namespace Insolence
{
    public class SaveUtils
    {
        ...
        private static void SavePlayer(string path)
        {
            ...
            path = Application.persistentDataPath + "/savegames/" + "placeholdere"; // player.GetComponent<CharacterStatus>().GetStatus()["name"];
        ...
        private static void LoadPlayer(string path)
        {
            if (File.Exists(path))
            {
                ...
                player.GetComponent<KinematicCharacterMotor>().SetPositionAndRotation(ConvertToVector3(objectState.position), ConvertToQuaternion(objectState.rotation));
        ...
        private static void SaveDynamicObjects(string path, string sceneName)
        {
            ...
            path = Application.persistentDataPath + "/savegames/" + "placeholder"; //player.GetComponent<CharacterStatus>().GetStatus()["name"];
        ...
        private static string IteratePath(string originalPath, string type, int i)
        {
            ...
            if (type == "obj")
            {
                ...
                {
                    path = Application.persistentDataPath + "/savegames/" + "placeholder" + "[" + i + "]"; //player.GetComponent<CharacterStatus>().name + "[" + i + "]";
                    i += 1;
